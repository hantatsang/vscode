...
class PerfModelContentProvider implements ITextModelContentProvider {
	...
	private _addSummaryTable(md: MarkdownBuilder, stats?: LoaderStats): void {
		...
		table.push(['window.loadUrl() => begin to require(workbench.native.main.js)', metrics.timers.ellapsedWindowLoadToRequire, '[main->renderer]', StartupKindToString(metrics.windowKind)]);
		table.push(['require(workbench.native.main.js)', metrics.timers.ellapsedRequire, '[renderer]', `cached data: ${(metrics.didUseCachedData ? 'YES' : 'NO')}${stats ? `, node_modules took ${stats.nodeRequireTotal}ms` : ''}`]);
